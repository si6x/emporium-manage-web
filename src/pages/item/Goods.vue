    ...
    >
      <template slot="items" slot-scope="props">
        ...
        <td class="justify-center layout px-0">
          ...
          <v-btn icon @click="offGoods(props.item.id)" v-if="props.item.saleable">下架</v-btn>
          <v-btn icon @click="onGoods(props.item.id)" v-else>上架</v-btn>
        </td>
      </template>
    </v-data-table>
    <!--弹出的对话框-->
    ...
    methods: {
      ...
      deleteGoods(goodId) {
        this.$message.confirm("确认要删除该商品吗？")
          .then(() => {
            this.$http.delete("/item/goods/" + goodId)
              .then(() => {
      ...
      offGoods(goodId) {
        this.$message.confirm("确认要下架该商品吗？")
          .then(() => {
            this.$http.post("/item/goods/offGoods/" + goodId)
              .then(() => {
      ...
      onGoods(goodId) {
        this.$message.confirm("确认要上架该商品吗？")
          .then(() => {
            this.$http.post("/item/goods/onGoods/" + goodId)
              .then(() => {
